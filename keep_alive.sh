...
# Detección multi-hosting (mismo orden que webhook_selfheal.php)
PING_URL="${APP_URL:-${WEBHOOK_BASE_URL:-${PUBLIC_URL:-${RENDER_EXTERNAL_URL:-}}}}"

# Heroku
if [ -z "$PING_URL" ] && [ -n "$HEROKU_APP_DEFAULT_DOMAIN_NAME" ]; then
    PING_URL="https://$HEROKU_APP_DEFAULT_DOMAIN_NAME"
fi
if [ -z "$PING_URL" ] && [ -n "$HEROKU_APP_NAME" ]; then
    PING_URL="https://${HEROKU_APP_NAME}.herokuapp.com"
fi

# Railway
if [ -z "$PING_URL" ] && [ -n "$RAILWAY_PUBLIC_DOMAIN" ]; then
    PING_URL="https://$RAILWAY_PUBLIC_DOMAIN"
fi

# Fly.io
if [ -z "$PING_URL" ] && [ -n "$FLY_APP_NAME" ]; then
    PING_URL="https://${FLY_APP_NAME}.fly.dev"
fi
